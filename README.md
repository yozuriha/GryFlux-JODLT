...
- Eigen3
- pthread
- Ascend CANN Toolkit / ACL
- 可用的 Ascend 运行环境

如果 CMake 无法自动找到 Ascend ACL，可以手动指定：

```bash
cmake -S . -B build -DASCEND_TOOLKIT_HOME=/usr/local/Ascend/ascend-toolkit/latest
```

### 2. 准备模型和数据

在src/app/ascend/config_jodlt.json中配置相关路径和参数

模型目录中应包含当前代码使用的 OM 文件：
图像数据放在 `data文件夹中`，`ImgDataSource` 会遍历目录中的常规文件并用 `cv::imread` 读取。
...
构建产物默认安装到 `install/`，可执行文件也会出现在构建目录的 `bin/` 下。
...
1. 初始化日志和 Ascend Runtime。
2. 创建 Host/Device 内存池。
3. 注册 YOLO、XFeat、match、encoder 推理资源。
4. 构建 JODLT DAG。
5. 从图像目录持续产生 `Packet`。
6. 执行异步流水线并输出统计信息。
...
Profiler 是编译期开关，默认不编译进二进制。需要使用：

```bash
bash build.sh --enable_profile
```

在 `GRYFLUX_BUILD_PROFILING=1` 时，`test_dag.cpp` 会调用：
...
时间线 JSON 可用于分析每个 packet 在各节点上的执行区间，定位推理节点、CPU 后处理节点或背压造成的瓶颈。

---

## 当前的性能分析：

![1781160371328](images/README/1781160371328.png)
